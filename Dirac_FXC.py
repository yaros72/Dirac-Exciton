...
class Dirac_Exciton(object):
    ...
    def exciton_hamiltonian(self,N,tau=1):
        self.N=N
        ...
        for i in range(N):
            for j in range(N):
                ...
                self.E[i,j]=e
                self.D[i,j]=d
        self.F=np.array(self.E<0,dtype=int)
                ...
                for n in range(2):
                    for m in range(2):
                        if (self.F[i,j,n]-self.F[i,j,m])!=0:
                            self.chi0+=2*self.P[i,j,n,m]*self.P[i,j,m,n]*(self.F[i,j,n]-self.F[i,j,m])/(self.E[i,j,n]-self.E[i,j,m])
        self.chi0/=self.NS               
